...
Inbox Cleanup evaluates Gmail, Exchange Web Services, and Standards-Based Mail metadata on device using the same read, age, multi-category, Pin, reply-evidence, and canonical mailbox-role rules. It offers only reviewed per-message moves to provider Trash through durable Pending Provider Actions. EWS connections participate after their required distinguished mailbox roles are verified, retain the stable provider search key while item and change identities rotate across moves, and stop ambiguous mutations for reconciliation instead of replaying them. Standards-Based connections participate only when Trash is mapped and UIDPLUS permits a recoverable identity-preserving move. IMAP actions retain mailbox, UIDVALIDITY, and UID evidence through reconciliation, while partial failures stay scoped to their Mailbox Connection. The review explains that automatic Undo depends on a supported restore-to-Inbox path and that provider Trash, never permanent erasure, remains the recovery boundary.
