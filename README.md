...
An expanded message also offers **View Message Source** as an explicit, Profile-protected action. The inspector fetches at most 25 MB only after that action, displays parsed headers separately from the raw MIME source, and preserves the provider-returned bytes for Copy Source and `.eml` export. Gmail uses its raw message resource, Standards-Based Mail uses the RFC 822 bytes returned by IMAP, Microsoft 365 uses the Graph MIME endpoint, and on-premises Exchange requests EWS `MimeContent`; a provider that cannot return exact bytes shows an honest metadata-only header fallback instead of reconstructing a message. Exact source bytes share the encrypted, bounded message-body cache and its account and connection cleanup boundaries, never enter Product Sync or the backend, and are dismissed when Profile Lock closes the protected mail surface.
