...
The Apple client detects a structured `text/calendar`, `application/ics`, or `text/x-vcalendar` MIME part in Gmail metadata without requesting its bytes. Only an explicit Add to Calendar action downloads the invitation through Gmail's authenticated attachment boundary, with a 1 MB limit, bounded iCalendar parsing, and an on-device Calendar Event Candidate. After a message body is already loaded, a provider-neutral on-device detector may also offer one prose event when a body within its scan limit contains exactly one explicit calendar date, including a year, and an explicit time; it never fetches or truncates a body for detection. Prose events always open the native Calendar editor for time-zone, duration, and location review without requiring full Calendar access, and a product-account-wide device-local fingerprint can warn about a possible duplicate but never selects or replaces an invitation-backed event. The Apple client requests full Calendar access only for structured invitation reads and updates, keeps UID mappings, fingerprints, candidate fields, and Calendar data on device, and sends none of them to TypeScript. An expanded message displays at most one proactive card, prioritized as Event, Unsubscribe, then Contact; End-to-End Encrypted Product Sync carries only the Feature Suggestion Preference enablement and opaque 14-day dismissals.
