...
5. Set a high-entropy, backend-only `GMAIL_ROUTING_KEY` in the Convex deployment. Convex uses it only to derive versioned HMAC routing digests from transient, verified Gmail addresses; neither the address nor the key is stored or returned.
