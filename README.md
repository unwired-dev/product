...
Inbox Cleanup evaluates Gmail, Microsoft Graph, and Standards-Based Mail metadata on device using the same read, age, multi-category, Pin, reply-evidence, and canonical mailbox-role rules. It offers only reviewed per-message moves to provider Trash through durable Pending Provider Actions. Microsoft Graph connections participate only after well-known Inbox and Trash roles are available, retain immutable message IDs across moves, and stop ambiguous mutations for reconciliation rather than replaying them. Standards-Based connections participate only when Trash is mapped and UIDPLUS permits a recoverable identity-preserving move. IMAP actions retain mailbox, UIDVALIDITY, and UID evidence through reconciliation, while partial failures stay scoped to their Mailbox Connection. The review explains that automatic Undo depends on a supported restore-to-Inbox path and that provider Trash, never permanent erasure, remains the recovery boundary.
