...
New Message is a separate bottom-right action from the Mail View bar and hides while the shared new-message, reply, reply-all, or forward composer is visible. The composer opens at 60% height by default, can expand only for its current presentation, and uses the Mail Profile's synchronized Compose Presentation Preference for later composers. It keeps the body and fixed action bar ahead of Subject and recipient fields, preserves the source Mailbox Connection for replies and forwards, and continuously autosaves each Draft into profile-isolated encrypted device storage. Close waits for a durable save, Discard confirms user-authored state, and Send removes the Draft only after Outbox admission. To, Cc, and Bcc remain distinct through validation, rendering, and submission by Gmail, SMTP, Microsoft Graph, Exchange Web Services, and the experimental mail engine. Recipient suggestions rank on-device correspondence and recency, already-permissioned Apple Contacts, and an optional device-authenticated provider directory without sending an address or query through the product backend. Rich-text authoring and encrypted attachment and inline-image assets remain the follow-on Draft representation described by issue #163.
