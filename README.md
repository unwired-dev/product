...
5. Set a high-entropy, backend-only `GMAIL_ROUTING_KEY` and its positive integer `GMAIL_ROUTING_KEY_VERSION` in the Convex deployment. Convex uses them only to derive versioned HMAC routing digests from transient, verified Gmail addresses; neither the address nor the key is stored or returned. To rotate, configure `GMAIL_ROUTING_PREVIOUS_KEY` and `GMAIL_ROUTING_PREVIOUS_KEY_VERSION` with the former values until every active route has renewed on the new version, then remove both previous-key variables.
