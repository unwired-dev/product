...
Gmail metadata also detects a structured `text/calendar`, `application/ics`, or `text/x-vcalendar` MIME part without requesting its bytes. Only an explicit Add to Calendar action downloads the invitation through Gmail's authenticated attachment boundary, with a 1 MB limit and bounded iCalendar parsing. The app requires review before EventKit creates, updates, or removes an event, requests full Calendar access only from that action, and keeps UID mappings and extracted values on device. An expanded message displays at most one proactive card, prioritized as Event, Unsubscribe, then Contact; encrypted Product Sync carries only enablement and opaque 14-day dismissals.
