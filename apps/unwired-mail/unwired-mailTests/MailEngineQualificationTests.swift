...
extension MailEngineQualificationEvent {
  func belongs(to connectionID: String, service: MailEngineService) -> Bool {
    ...
    case .bodyFetchCancelled, .bodyFetchStarted, .bodyPartsRequested, .closed, .copyReceived,
      ...
      .sentAppendReceived, .submissionContentAccepted, .submissionReceived, .submissionStarted,
...
struct MailEngineQualificationContract {
  ...
  private func verifySuccessfulSTARTTLSIDLERecovery() async throws {
    ...
    let inbox = MailEngineMailboxIdentity("INBOX")
    let session = try await connect(
      fixture: .idleDisconnectThenRecover(maximumReconnectTLSVersion: .tls12),
      connectionID: connectionID,
      imapTransportMode: .startTLS
    ).session
    do {
  ...
  private func verifyOverlappingConnectionIsolation() async throws {
    ...
    try await verifySecondIdleCancellationPreservesSession(secondTask, session: second)
  ...
  private func verifyCloseAfterSMTPContentIsAmbiguous() async throws {
    ...
    let task = Task {
      try await session.submit(
        ...
        rawMessage: Data("Subject: Close after DATA\r\n\r\nPrivate body".utf8)
      )
    }
...
private actor ScriptedMailEngineState {
  ...
  func waitForOverlappingMutationStarts(timeout: Duration) async throws {
  ...
  func waitForOverlappingConnectionSetupStarts(timeout: Duration) async throws {
    try await waitForEvents(2, timeout: timeout) {
      if case .authenticationStarted(let connectionID, service: .imap) = $0 {
        return connectionID == "setup-connection-one" || connectionID == "setup-connection-two"
      }
      return false
    }
...
private struct ScriptedMailEngine: MailEngine {
  ...
  private func establish(
    ...
    if case .overlappingConnectionSetup = fixture, service == .imap {
      try await state.waitForOverlappingConnectionSetupStarts(timeout: .seconds(2))
    }
...
private actor ScriptedMailEngineSession: MailEngineSession {
  ...
  private func handleInvalidIDLEEvent(
    ...
    if case .invalidIdleChangedUID(let uid) = fixture {
      guard (1...4_294_967_295).contains(uid) else {
        throw MailEngineUIDMappingError.invalidUID
      }
      let event = MailEngineIdleEvent.changedUIDs([uid])
      await onEvent(event)
      await state.record(.idleEventDelivered(connectionID: connectionID, event: event))
      return true
    }
