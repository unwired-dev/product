...
The current Account & Devices implementation slice includes Product Account identity, Trusted Device listing and rename, Recovery Key generation or replacement after recent authentication, and current-device sign-out through the mail shell's full local-cleanup path. The backend binds sign-out unregistration to the caller's current device identifier; remote device revocation and Product Account deletion remain separate security-sensitive slices tracked by issues #129 and #130.
