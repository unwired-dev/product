...
Status: cross-device Send Reminder plus Gmail, Microsoft Graph, On-Premises Exchange, and Standards-Based Scheduled Send delivery, revision-fenced Outbox management, lifecycle hardening, and deterministic local evidence implemented; protected provider compatibility evidence remains required before release
...
- Implemented: keep new automatic scheduling behind `ScheduledSendReleasePolicy` in Release builds while Debug and Mail Test builds exercise the complete local path. Existing commitments remain editable while the release gate is closed. Issue #386 owns the single protected-provider release decision.
- Run Apple lint, focused Swift Testing suites, Convex lint/typecheck/tests, Core Mail Loop coverage, performance and Release gates, then complete protected provider compatibility checks for every send-capable adapter before setting `protectedProviderCompatibilityComplete` to `true`.
