...
Status: the secure GreenMail smoke foundation, disposable Mail Test Device, Apple app bootstrap, Synthetic Test Message visibility assertion, on-demand message-content scenario, and persistent Manual Mail Sandbox are available; broader scenarios, the required pull-request gate, and provider compatibility remain planned.
