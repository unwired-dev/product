...
- Secure-transport qualification must reject invalid certificates and hostnames, reject failed or downgraded STARTTLS upgrades, and prove authentication cannot begin before TLS is established. Deterministic TLS-1.0-only and TLS-1.1-only servers must fail through both implicit TLS and STARTTLS, while TLS-1.2-or-newer servers must succeed, so the minimum restored by ADR-0028 is exercised.
- Connection-lifecycle qualification must run overlapping sessions for two mailbox connections with distinct credentials and distinguishable mailbox data. Commands and results must remain connection-scoped, cancelling one connection's IDLE or in-flight operation must not cancel, disconnect, or deliver callbacks to the other, and a later service setup failure must close any service that was already authenticated.
...
- SwiftMail protocol trace and debug output may contain private data, so production logging must discard all authentication material, mailbox identifiers, and message content. Tests must capture every candidate-owned trace and debug path as well as the supplied product logger and prove passwords, bearer tokens, authentication exchanges, mailbox identifiers, and message content cannot reach any captured output.
...
SMTP cancellation and `DATA` rejection, exact SMTP and Sent-append payloads,
