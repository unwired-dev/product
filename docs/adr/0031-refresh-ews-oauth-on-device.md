...
# Refresh On-Premises Exchange Connection OAuth authorization on device

An On-Premises Exchange Connection uses OAuth 2.0 authorization code with PKCE through the system authentication session. The Apple target supplies the organization's HTTPS authorization endpoint, HTTPS token endpoint, public client identifier, callback scheme, and EWS scope. Unwired does not accept or bundle a client secret.
...
Each operator must supply and rotate the provider endpoints and public client identifier. A rejected refresh grant presents a user-visible reauthorization prompt. Removing the device's Keychain authorization record removes mailbox access on that device while leaving the synchronized connection in place.
