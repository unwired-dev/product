...
The first implementation is a deterministic local rule-based engine. Gmail metadata sync sends subject, snippet, and selected headers as Minimized Classification Input. The engine explicitly requests body text when metadata is insufficient; only then may the categorization service read from the encrypted On-Demand Body Cache. Historical and already categorized messages bypass the engine.
