...
- Copy and move require a complete validated `COPYUID` mapping. MOVE is unavailable without
  UIDPLUS. The reduced-capability path validates COPYUID before marking the exact source UIDs as
  deleted and uses only `UID EXPUNGE`; it never invokes unrestricted expunge.
...
Run Apple formatting, linting, and tests through the repository commands:
