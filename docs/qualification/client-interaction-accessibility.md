...
| Dynamic Type, long localized text, and reduced motion | `testNativePlatformHierarchyRemainsUsableWithAccessibilityTextAndReducedMotion()` uses the accessibility-extra-extra-extra-large category, doubled localization, and a Debug-only launch environment that disables animations (a stricter motion stress than reduced motion). It runs focused Dynamic Type, hit-region, description, and clipping audits and retains evidence. |
